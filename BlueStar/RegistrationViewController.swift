...
class RegistrationViewController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UITextViewDelegate, ProductTokenViewDelegate, UIPickerViewDataSource , AddressActionProtocol {
...
//        alternateNoCheckBoxButton.isHidden = true //Commented by Ashish
...
//        updateTextFieldsYcoordinate()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        /* setContentSize()                     Commented By Ashish
...
//        }
        ...
        textFieldView.leftImageView.setImageColor(color: imgColor)
...
//        ageGroupTextField = setTextField(&ageGroupTextField, placeholder:"Age Group", leftImage: IMG_User, yAxis: 30+70*3)
    ...
    // MARK: - setup dropdown for customer ID
...
//        buttonSubmit = UIButton(frame: CGRect(x: 10, y: 70+productTokenField.frame.origin.y, width: UIScreen.main.bounds.size.width-20, height: 50))
        buttonSubmit = UIButton(frame: CGRect(x: UIScreen.main.bounds.size.width/4 , y: 70+alternateMobileNoTextField.frame.origin.y, width: 200, height: 50))
        
        //        let button = UIButton(frame: CGRect(x: 10, y: 70+70*11, width: UIScreen.mainScreen().bounds.size.width-20, height: 50))
        buttonSubmit.setTitle("Next", for: UIControl.State())
...
//        }
    ...
    // MARK: - New address event callback
    func onAddNewAddress() {
            print("New address event hit")
        //open address view controller
        moveToAddAddressView()
    }
    
    // MARK: - Customer id dropdown interaction
...
//        resConfigTextField.resignFirstResponder()
    ...
    @objc func submitButtonAction(_ sender: AnyObject) {
        ...
        if validateFields() {
            UserDefaults.standard.setValue(self.selectedTitle, forKey: "strTitle")
            UserDefaults.standard.setValue(self.firstNameTextField.textField.text!, forKey: "strFirstName")
            UserDefaults.standard.setValue(self.lastNameTextField.textField.text!, forKey: "strLastName")
            UserDefaults.standard.setValue(self.genderTextField.text!, forKey: "strGender")
            UserDefaults.standard.setValue(self.selectedAge!, forKey: "strSelectedAge")
            UserDefaults.standard.setValue(self.primaryMobileNoTextField.textField.text, forKey: "strPrimaryMobileNumber")
            UserDefaults.standard.setValue(self.alternateMobileNoTextField.textField.text, forKey: "strAlternateMobilerNumber")
            
            let controller:RegisterAddressViewController = self.storyboard!.instantiateViewController(withIdentifier: "RegisterAddressView") as! RegisterAddressViewController
            let add = (PlistManager.sharedInstance.getValueForKey("user") as? NSDictionary)?.value(forKey: "addresses") as! NSArray
            print("Get User Addresses :\(add)")
            controller.addresses = (PlistManager.sharedInstance.getValueForKey("user") as? NSDictionary)?.value(forKey: "addresses") as! NSArray
            // setup delegate for new address selection
            controller.addressProtocol1 = self
            //UserAddressesDatabaseModel.shareInstance.getUserAddrress(PlistManager.sharedInstance.getValueForKey("mobileNumber") as! String)
...
//        if validateFields() {
//            let controller:RegisterTwoVC = self.storyboard!.instantiateViewController(withIdentifier: "RegisterTwoVC") as! RegisterTwoVC
//            controller.view.frame = self.view.bounds;
//            //        controller.view.backgroundColor = UIColor(white: 0.1, alpha: 0.65)
//
//            UserDefaults.standard.setValue(self.selectedTitle, forKey: "strTitle")
...
//            ProductBL.shareInstance.selectedProducts = mutableArray
//            self.productTokenField.tokens = mutableArray  Commented Ashish
//            self.productTokenField.reloadTokeData()
...
//        setContentSize()
    ...
    // MARK: - Move to add address controller
    func moveToAddAddressView() {
