...
- (CGSize)intrinsicContentSize
{
    ...
    // NSLog(@"tokenRect %@",NSStringFromCGSize(totalRect.size));
//    if (totalRect.size.height>0) {
        //inform view about its total height
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(tokenViewSizeForAllTokens:)]) {
            [self.dataSource tokenViewSizeForAllTokens:totalRect.size];
        }
...
- (void)reloadData
{
    ...
    if (self.dataSource) {
        ...
        for (int i = 0 ; i < count ; i++) {
            ...
            tokenView.layer.borderColor = [UIColor blackColor].CGColor;//[UIColor colorWithRed:(152/255.0) green:(179/255.0) blue:(216/255.0) alpha:1.0].CGColor;
