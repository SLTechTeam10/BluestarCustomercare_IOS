...
class SafetyTipViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int{
        return SafetyTipData.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        let cell : SafetyTipTableViewCell = (tableView.dequeueReusableCell(withIdentifier: "SafetyTipCell", for: indexPath) as? SafetyTipTableViewCell)!
        let data = SafetyTipData[indexPath.row]
        cell.safetyTipLabel.text = data["tip"]!
        cell.safetyTipLabel.textColor = UIColor(netHex: 0x666666)
        cell.borderLabel.backgroundColor = UIColor(netHex: 0xDEDFE0)
        return cell
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        updateCells()
        let currentCell = tableView.cellForRow(at: indexPath)! as! SafetyTipTableViewCell
        let data = SafetyTipData[indexPath.row]
        let viewHeight = calculateHeightForString(data["detail"]!)
        print(viewHeight)
        var answerView = FAQAnswerView()
        answerView = Bundle.main.loadNibNamed("FAQAnswerView", owner: self, options: nil)?[0] as! FAQAnswerView
        answerView.answerTextView.text = data["detail"]
    ...
    func updateCells() {
        for index in 0...SafetyTipData.count {
